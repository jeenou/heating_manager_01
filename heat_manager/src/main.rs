...
async fn make_post_request_light(url: &str, entity_id: &str, token: &str, brightness: u64) -> Result<(), Box<dyn std::error::Error>> {
    // Construct the request headers
    let mut headers = HeaderMap::new();
    headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
    headers.insert(
...
async fn main() {
    ...
    let brightness_values: Vec<u64> = vec![10, 255, 100, 50, 0, 230, 40, 60, 125, 255];
